...
mod mldsa_benches {
    ...
    fn sign(c: &mut Criterion) {
        ...
        c.bench_function("ML-DSA signing", move |b| b.iter(|| keypair.sign(&msg, Some(&ctx), crystals_dilithium::RandomMode::Deterministic)));
    }
    ...
    fn verify(c: &mut Criterion) {
        ...
        random_bytes(&mut msg, 32);
        random_bytes(&mut ctx, 32);
        let sig = keypair.sign(&msg, Some(&ctx), crystals_dilithium::RandomMode::Deterministic).unwrap();
