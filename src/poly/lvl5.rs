...
pub fn decompose(a: &Poly) -> (Poly, Poly) {
    ...
    for i in 0..N {
        (a0.coeffs[i], a1.coeffs[i]) = rounding::lvl5::decompose(a.coeffs[i]);
    }
    (a0, a1)
}
...
pub fn make_hint(a0: &Poly, a1: &Poly) -> (Poly, i32) {
    ...
    let mut s: i32 = 0;
    for i in 0..N {
        hint.coeffs[i] = rounding::lvl5::make_hint(a0.coeffs[i], a1.coeffs[i]);
        s += hint.coeffs[i];
    }
    (hint, s)
}
...
pub fn use_hint(a: &Poly, hint: &Poly) -> Poly {
    ...
    for i in 0..N {
        result.coeffs[i] = rounding::lvl5::use_hint(a.coeffs[i], hint.coeffs[i]);
    }
    result
}
