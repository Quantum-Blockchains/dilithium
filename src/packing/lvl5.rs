...
pub fn pack_pk(pk: &mut [u8], rho: &[u8], t1: &Polyveck) {
    ...
    for i in 0..K {
        poly::t1_pack(
            &mut pk[params::SEEDBYTES + i * params::POLYT1_PACKEDBYTES..],
            &t1.vec[i],
        );
    }
}
...
pub fn unpack_pk(rho: &mut [u8], t1: &mut Polyveck, pk: &[u8]) {
    ...
    for i in 0..K {
        poly::t1_unpack(
            &mut t1.vec[i],
            &pk[params::SEEDBYTES + i * params::POLYT1_PACKEDBYTES..],
        );
    }
}
...
pub fn pack_sk(
    ...
    for i in 0..L {
        poly::lvl5::eta_pack(
            &mut sk[idx + i * params::lvl5::POLYETA_PACKEDBYTES..],
            &s1.vec[i],
        );
    }
...
pub fn unpack_sk(
    ...
    for i in 0..L {
        poly::lvl5::eta_unpack(
            &mut s1.vec[i],
            &sk[idx + i * params::lvl5::POLYETA_PACKEDBYTES..],
        );
    }
    idx += L * params::lvl5::POLYETA_PACKEDBYTES;
...
pub fn pack_sig(sig: &mut [u8], c: Option<&[u8]>, z: &Polyvecl, h: &Polyveck) {
    ...
    for i in 0..L {
        poly::lvl5::z_pack(
            &mut sig[idx + i * params::lvl5::POLYZ_PACKEDBYTES..],
            &z.vec[i],
        );
    }
...
pub fn unpack_sig(c: &mut [u8], z: &mut Polyvecl, h: &mut Polyveck, sig: &[u8]) -> bool {
    ...
    for i in 0..L {
        poly::lvl5::z_unpack(
            &mut z.vec[i],
            &sig[idx + i * params::lvl5::POLYZ_PACKEDBYTES..],
        );
    }
    idx += L * params::lvl5::POLYZ_PACKEDBYTES;
    ...
    let mut k: usize = 0;
    for i in 0..K {
        if sig[idx + params::lvl5::OMEGA + i] < k as u8
            || sig[idx + params::lvl5::OMEGA + i] > params::lvl5::OMEGA as u8
        {
            return false;
        }
